...
class OrderTransactionManager(models.Manager):
    def create_new(self, order, amount, success=None, transaction_status=None):
        ...
        final_hash = hashlib.sha1((order_hash + email_hash).encode('urf-8')).hexdigest()[:10]
        merchant_order_id = "%s"%(final_hash)
        payments_prepare(merchant_order_id, amount)
        transaction = self.model(
            order=order,
            merchan_order_id=merchant_order_id,
            amount=amount
        )
...
class OrderTransaction(models.Model):
    ...
    type = models.CharField(max_length=220, blank=True)
